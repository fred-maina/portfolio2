...
export default function App(){
  ...
  const [desktopBooking,setDesktopBooking]=useState(()=>typeof window!=='undefined'&&window.matchMedia('(min-width: 801px)').matches);
  useEffect(()=>{const query=window.matchMedia('(min-width: 801px)');const update=()=>setDesktopBooking(query.matches);update();query.addEventListener('change',update);return()=>query.removeEventListener('change',update)},[]);
  ...
  return <div className="site-shell">
    ...
    <main id="top">
      ...
      <section id="coffee" className="coffee-section page-section"><Reveal className="coffee-copy"><Coffee/><p className="section-number">05 / Coffee chat</p><h2>{coffee.heading[0]}<br/>{coffee.heading[1]}</h2><div className="coffee-description"><p>{coffee.description}</p><p className="coffee-meta">{coffee.details}</p><a className="booking-jump desktop-booking-link" href="#booking-calendar" onClick={()=>trackEvent('view_available_times')}>View available times <ArrowRight size={16}/></a></div></Reveal>{desktopBooking&&<Reveal className="calendar-wrap booking-shell"><iframe id="booking-calendar" src={coffee.calendarUrl} title={`Book a coffee chat with ${profile.name}`} width="100%" height="760" frameBorder="0" loading="lazy"/></Reveal>}<div className="mobile-booking-card"><Coffee size={24}/><div><h3>Choose a time on Google Calendar</h3></div><a href={coffee.calendarUrl} target="_blank" rel="noreferrer" onClick={()=>trackEvent('open_booking_page',{device:'mobile',placement:'booking_card'})}>Open booking calendar <ArrowUpRight size={17}/></a></div></section>
    </main>
